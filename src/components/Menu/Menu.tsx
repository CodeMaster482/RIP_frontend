...
const Menu: FC<MenuProps> = ({setPage}) => {
    ...
    // const {draftID} = useAppSelector(state => state.banknoteReducer)
    ...
    useEffect(() => {
        setPage()
        //dispatch(fetchCurrentOperation())
    }, []);
    ...
    return (
        <Container className="text-center mt-5">
            <div className="menu rounded p-4">
                <h2 className="mb-2">Меню</h2>
                <div className="d-flex flex-column">
                    <Link to="/companies" className="btn btn-outline-info my-2 rounded-pill">Купюры</Link>
