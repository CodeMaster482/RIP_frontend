...
import {fetchOperations, fetchOperationsFilter, moderatorUpdateStatus} from "../../store/reducers/ActionCreator.ts";
...
const RequestView: FC<RequestViewProps> = ({setPage}) => {
    ...
    const {operation, error, success} = useAppSelector((state) => state.operationReducer);
    ...
    const [filteredOperations, setFilteredOperations] = useState<IOpearation[] | null>(null);
    const [filteredByUsers, setFilteredUsers] = useState<IOpearation[] | null>(null);
                    ...
                    {operation &&
                        <table className='table-operations'>
                            ...
                            {filteredOperations && role == '0'
                                ...
                                : (filteredByUsers ? filteredByUsers : operation.operations).map((operation) => (
                                    <tr key={operation.id} onClick={(event) => clickCell(operation.id, event)}>
                                        {/*<td>{operation.id}</td>*/}
                                        <td>{operation.operation_name || 'Не задано'}</td>
                                        <td>{operation.status_check || 'Не рассмотрен'}</td>
                                        <td>{checkData(operation.created_at)}</td>
                                        ...
                                        <td>{operation.status}</td>
                                        <td>
                                            {operation.status == 'сформирован' && role == '2' && (
